-- 双联抗板用药识别：构建原始明细、事件表及供主脚本使用的视图
BEGIN
    EXECUTE IMMEDIATE 'DROP TABLE CAD_DUL_ANTIPLATELET_DRUG PURGE';
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -942 THEN
            RAISE;
        END IF;
END;
/

CREATE TABLE CAD_DUL_ANTIPLATELET_DRUG
AS
SELECT a.*
FROM KSQC_2024.SQC_WM_PRESCRIPTIONS_ALL a
JOIN (
    SELECT
        GLOBAL_INDEX,
        ORGAN_NAME,
        TRUNC(DRUG_DATE) AS DRUG_DAY
    FROM
        KSQC_2024.SQC_WM_PRESCRIPTIONS_ALL
    WHERE
        CAT_CODE = 'XB01AC'
    GROUP BY
        GLOBAL_INDEX,
        ORGAN_NAME,
        TRUNC(DRUG_DATE)
    HAVING
        COUNT(DISTINCT DRUG_NAME) > 1
) b
ON a.GLOBAL_INDEX = b.GLOBAL_INDEX
AND a.ORGAN_NAME = b.ORGAN_NAME
AND TRUNC(a.DRUG_DATE) = b.DRUG_DAY
WHERE
    a.CAT_CODE = 'XB01AC'
ORDER BY
    a.ORGAN_NAME,
    b.DRUG_DAY,
    a.GLOBAL_INDEX,
    a.DRUG_NAME;

-- 每日双联抗板事件汇总表，便于后续事件级分析
BEGIN
    EXECUTE IMMEDIATE 'DROP TABLE CAD_DUL_ANTIPLATELET_EVENTS PURGE';
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -942 THEN
            RAISE;
        END IF;
END;
/

CREATE TABLE CAD_DUL_ANTIPLATELET_EVENTS AS
SELECT
    GLOBAL_INDEX,
    ORGAN_NAME,
    TRUNC(DRUG_DATE) AS EVENT_DATE,
    LISTAGG(DISTINCT DRUG_NAME, '; ') WITHIN GROUP (ORDER BY DRUG_NAME) AS DRUG_LIST,
    COUNT(DISTINCT DRUG_NAME) AS DRUG_COUNT
FROM
    CAD_DUL_ANTIPLATELET_DRUG
GROUP BY
    GLOBAL_INDEX,
    ORGAN_NAME,
    TRUNC(DRUG_DATE);

-- 将派生表暴露为视图供CAD定义脚本使用
CREATE OR REPLACE VIEW cad_dual_antiplatelet_therapy AS
SELECT
    d.GLOBAL_INDEX,
    d.ORGAN_NAME AS encounter_id,
    TRUNC(d.DRUG_DATE) AS therapy_start_date,
    d.DRUG_NAME,
    d.DRUG_DATE,
    d.CAT_CODE
FROM CAD_DUL_ANTIPLATELET_DRUG d;
